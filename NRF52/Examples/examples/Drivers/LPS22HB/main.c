/**
 * @brief       main.c
 * @details     This example shows how to work with the external device: LPS22HB. Every 1 seconds, a new
...
int main(void)
{
  ...
  aux  =   LPS22HB_SetPowerMode ( myLPS22HB_I2C_parameters, RES_CONF_LC_EN_NORMAL_MODE ); 

  /* Get device ID  */
  aux  =   LPS22HB_GetDeviceID ( myLPS22HB_I2C_parameters, &myLPS22HB_Data );

  /* Interrupt generation disabled  */
  aux  =   LPS22HB_SetInterruptMode ( myLPS22HB_I2C_parameters, INTERRUPT_CFG_AUTORIFP_NORMAL, INTERRUPT_CFG_RESET_ARP_NORMAL, INTERRUPT_CFG_AUTOZERO_NORMAL, INTERRUPT_CFG_RESET_AZ_NORMAL, 
                                      INTERRUPT_CFG_DIFF_EN_DISABLED, INTERRUPT_CFG_LIR_NOT_LATCHED, INTERRUPT_CFG_PLE_DISABLED, INTERRUPT_CFG_PHE_DISABLED );
  ...
  while( 1 )
  {
    ...
    if ( myState == 1UL )
    {
      ...
      aux  =   LPS22HB_TriggerOneShot ( myLPS22HB_I2C_parameters );
      ...
      }while( ( myLPS22HB_Data.status & ( STATUS_REG_P_DA_MASK | STATUS_REG_T_DA_MASK ) ) != ( STATUS_REG_P_DA_NEW_DATA | STATUS_REG_T_DA_NEW_DATA ) ); // Dangerous!!! The uC may get stuck here...
                                                                                                                                                        // [WORKAROUND] Insert a counter

 ...
 * @details     One channel. Channel zero at 125ns.
 *
 *              Timer0:
 *                  * Prescaler:            1   ( f_Timer0 = 8MHz ( PCLK16M ) ).
 *                  * 32-bits mode.
 *                  * Interrupt Disabled.
 *
 *                 --- Channel 0:
 *                  * Overflow:             ( 1 * (f_Timer0)^(-1) ) = ( 1 * (8MHz)^(-1) ) = 125ns
 *
 *
 * @param[in]    N/A.
 *
...
void conf_TIMER0  ( void )
{
  ...
  NRF_TIMER0->PRESCALER   =   1UL;                                                                        // f_Timer0 = ( 16MHz / 2^1 ) = 8MHz
  ...
  NRF_TIMER0->CC[0]       =   ( 1UL );                                                                    // ( 1 * (f_Timer0)^(-1) ) = ( 1 * (8MHz)^(-1) ) ~ 125ns
